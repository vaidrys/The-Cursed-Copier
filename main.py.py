...
# DONNEES DU JEU 
game_title = "L'Epopée du Copieur Maudit"
...
locations = {
    "Village" : {
        "desc" : "Tu te trouves dans un vilage poussiéreux rempli de développeurs fatigués.",
    ...
    "Forêt Sombre" : {
        ...
        "actions" : ["Explorer (risque de combat)", "Retourner au Village", "Se reposer"]
    }, 
    "Combat" : {} 
}
...
def append_text(msg, color="white") :
    ...
    text_area.tag_add(color, "end-1l", "end")
...
def level_up() : 
    if player["xp"] >= player["xp_to_next"] : 
        ...
        append_text(f"!!! NIVEAU {player['level']} !!! Tes stats augmentent !", "green")
...
def combat_action(action) : 
    ...
    if current_enemy["hp"] > 0 : 
        dmg = max(1, current_enemy["atk"] - player["defense"] + random.randit(-2, 3))
        player["hp"] -= dmg 
    ...
    if player["hp"] <= 0 : 
        ...
        messagebox.showerror(game_title, "Game Over\nTu as été vaincu par un bug...")
        root.quit()
        return
    
    if current_enemy["hp"] <= 0 : 
        append_text(f"\n Victoire ! +{current_enemy['xp']} XP +{current_enemy['gold']} or", "green")
    ...
    for act in actions: 
        if act == "Parler au marchand" : 
            ...
            def go_forest(): 
                ...
                update_stats()
                show_actions("Forêt Sombre")
            ttk.Button(action_frame, text=act, command=go_forest).pack(fill="x", pady=3)
        elif act == "Retourner au Village" : 
            def go_village():
...
# Zone stats 
stats_frame = tk.Frame(main_frame, bg="#161b22", width=220)
stats_frame.pack(side="right", fill="y", padx=5)
stats_frame.pack_propagate(False)

tk.Label(stats_frame, text="SATISTIQUES", font=("Consolas", 12, "bold"), bg="#161b22", fg="#58a6ff").pack(pady=8)
stats_text = tk.Text(stats_frame, font=("Consolas", 10), bg="#0d1117", fg="white", borderwidth=0, height=12)
...
# Démarrage
append_text("Bienvenue dans L'Epopée du Copieur Maudit !", "cyan")
